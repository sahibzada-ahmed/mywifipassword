...
show_wifi_password() {
    # Check for connected Wi-Fi network
    ...
    if [ -n "$password" ]; then
        echo -e "${random_color}Wi-Fi Password: $password${reset_color}"
    else
        echo -e "${random_color}Unable to retrieve the password.${reset_color}"
    fi
}
